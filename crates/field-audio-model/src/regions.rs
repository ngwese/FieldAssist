...
use crate::buffer::{ChannelScope, Region, RegionId};

/// Well-known name for the primary selection collection.
pub const SELECTION_COLLECTION: &str = "selection";

/// Which inclusive endpoint of a region.
...
/// Named bag of regions (selection, edits, etc.).
...
impl RegionCollection {
    /// Create an empty collection with `name`.
    ...
    /// Whether any region covers `(sample, channel)`.
    ...
    /// First half-open edit span, if any.
    pub fn first_span(&self) -> Option<(u64, u64)> {
        self.edit_spans().into_iter().next()
    }

    /// Inclusive bounding box `(start, end)` across all regions.
    ...
    /// Append `region` and normalize.
    pub fn push(&mut self, region: Region) {
        self.regions.push(region);
        self.normalize();
    }

    /// Replace all regions with a single `region`.
    ...
    /// Copy regions from `other`, allocating fresh ids via `next_id`.
    ...
    /// Allocate an id, push a region, and return the surviving id after merge.
    pub fn alloc_push(
        &mut self,
        start: usize,
        end: usize,
    ...
    /// Clamp all endpoints into `0..=last_sample`.
    ...
    /// Remap inclusive endpoints; drop regions when `map` returns `None`.
    ...
    /// Nearest region endpoint to `sample`, optionally filtered by `scope`.
