...
impl MonitorFoaFuma {
	...
	pub fn build_user_interface_static(ui_interface: &mut dyn UI<FaustFloat>) {
		...
		ui_interface.add_horizontal_slider("Monitor/Gain", ParamIndex(2), 0.0, -9e+01, 12.0, 0.1);
	...
	pub fn compute(
		...
		for (((((input0, input1), input2), input3), output0), output1) in zipped_iterators {
			...
			let mut fTemp0: F32 = (*input1) as F32;
			let mut fTemp1: F32 = (if iSlow2 != 0 {fTemp0} else {(if iSlow3 != 0 {fTemp0} else {-((*input3) as F32)})});
			...
			let mut fTemp7: F32 = 1.0 * (*input0) as F32 + 0.8660254 * (fTemp1 * fTemp3 + fTemp5 * fTemp6);
