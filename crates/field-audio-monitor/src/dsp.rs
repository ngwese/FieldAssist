...
//! Faust DSP adapters implementing [`MonitorDsp`].
...
/// Planar audio DSP used by [`crate::MonitorHost`].
pub trait MonitorDsp: Send {
    /// Chain this instance implements.
    fn chain(&self) -> MonitorChain;
    /// Number of input planes.
    fn num_inputs(&self) -> usize;
    /// Number of output planes.
    fn num_outputs(&self) -> usize;
    /// Process `frames` of planar audio.
    fn compute(&mut self, frames: usize, inputs: &[&[f32]], outputs: &mut [&mut [f32]]);
    /// Set a Faust parameter by address.
    fn set_param(&mut self, address: &str, value: f32);
    /// Read a Faust parameter by address.
    fn get_param(&self, address: &str) -> Option<f32>;
    /// Embedded Faust UI JSON for this chain.
    fn ui_json(&self) -> &'static str;
    /// Passive (meter) parameter addresses.
    fn meter_addresses(&self) -> &[String];
    /// Active control addresses (non-meters).
    fn control_addresses(&self) -> Vec<String> {
        Vec::new()
    }
}

/// Construct the Faust (or test) DSP for `chain` at `sample_rate`.
...
impl IdentityDsp {
    /// Create an identity DSP for `chain`'s input count.
