...
//! Drag-drop layout helpers for workflow chrome.
...
/// Collect drag-drop workflows into rows for the default `"drag-drop"` scope.
pub fn layout_drop_targets(workflows: &[WorkflowMeta]) -> DropLayout {
    layout_drop_targets_for_scope(workflows, SCOPE_DRAG_DROP)
}

/// Collect workflows for `scope` into rows.
pub fn layout_drop_targets_for_scope<'a>(
...
/// Workflows bound to the `"menu"` scope as `(name, display_name)` pairs.
pub fn workflows_for_menu(workflows: &[WorkflowMeta]) -> Vec<(String, String)> {
...
mod tests {
    ...
    fn layout_groups_rows_without_padding() {
        ...
        let layout = layout_drop_targets_for_scope(&workflows, SCOPE_DRAG_DROP);
